...
violation strings. Collect new rules by appending to the relevant RULES list
...
# ---------------------------------------------------------------------------
# Cash account validator
# ---------------------------------------------------------------------------

def validate_account_file(path: Path, conn_id: str) -> list[Violation]:
    """Run all rules against one cash account JSON file."""
    try:
...
# ---------------------------------------------------------------------------
# Investment account validators
# ---------------------------------------------------------------------------

def validate_investment_account_file(path: Path, conn_id: str) -> list[Violation]:
    """Validate one investment account JSON file (metadata + transactions)."""
    try:
    ...
    acctid = data.get("id") or path.stem
    violations: list[Violation] = []

    for field, label in [("id", "id"), ("name", "name"), ("broker_id", "broker_id"),
                         ("conn_id", "conn_id")]:
        if not data.get(field, ""):
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                                        rule=f"inv_acct_{field}_nonempty",
                                        message=f"{label} is empty or missing"))

    txns = data.get("investment_transactions")
    if not isinstance(txns, list):
        violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                                    rule="inv_acct_transactions_is_list",
                                    message=f"investment_transactions is {type(txns).__name__}"))
        return violations
    ...
    for txn in txns:
        ...
        if txn_id and txn_id in seen_ids:
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=txn_id,
                                        rule="inv_txn_ids_unique",
                                        message=f"duplicate investment transaction id {txn_id!r}"))
        if txn_id:
            seen_ids.add(txn_id)

        if not txn.get("id", ""):
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                                        rule="inv_txn_id_nonempty",
                                        message="investment_transaction id is empty or missing"))
        if not txn.get("trntype", ""):
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=txn_id or None,
                                        rule="inv_txn_trntype_nonempty",
                                        message="trntype is empty or missing"))
        trade_date = txn.get("trade_date")
        if not isinstance(trade_date, int):
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=txn_id or None,
                                        rule="inv_txn_trade_date_int",
                                        message=f"trade_date {trade_date!r} is not an integer"))

    return violations


def validate_positions_file(path: Path, conn_id: str, acctid: str) -> list[Violation]:
    """Validate one positions snapshot file."""
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as e:
        return [Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                          rule="json_parse", message=str(e))]

    positions = data.get("positions")
    if not isinstance(positions, list):
        return [Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                          rule="positions_is_list",
                          message=f"positions is {type(positions).__name__}")]

    violations: list[Violation] = []
    for pos in positions:
        sec_id = str(pos.get("security_id") or "")
        if not sec_id:
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=None,
                                        rule="pos_security_id_nonempty",
                                        message="position security_id is empty or missing"))
        if not pos.get("security_id_type", ""):
            violations.append(Violation(conn_id=conn_id, acctid=acctid, txn_id=sec_id or None,
                                        rule="pos_security_id_type_nonempty",
                                        message="position security_id_type is empty or missing"))
    return violations


def validate_securities_file(path: Path) -> list[Violation]:
    """Validate the shared securities registry."""
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as e:
        return [Violation(conn_id="securities", acctid="registry", txn_id=None,
                          rule="json_parse", message=str(e))]

    securities = data.get("securities")
    if not isinstance(securities, list):
        return [Violation(conn_id="securities", acctid="registry", txn_id=None,
                          rule="securities_is_list",
                          message=f"securities is {type(securities).__name__}")]

    violations: list[Violation] = []
    for sec in securities:
        uid = str(sec.get("uniqueid") or "")
        if not uid:
            violations.append(Violation(conn_id="securities", acctid="registry", txn_id=None,
                                        rule="sec_uniqueid_nonempty",
                                        message="security uniqueid is empty or missing"))
        if not sec.get("uniqueid_type", ""):
            violations.append(Violation(conn_id="securities", acctid=uid or "registry",
                                        txn_id=None,
                                        rule="sec_uniqueid_type_nonempty",
                                        message="security uniqueid_type is empty or missing"))
    return violations


# ---------------------------------------------------------------------------
# Top-level cache validator
# ---------------------------------------------------------------------------

def validate_cache(cache_dir: Path) -> list[Violation]:
    """Validate all account files and investment data in cache_dir."""
    ...
    # Investment accounts + position snapshots
    investment_dir = cache_dir / "investment"
    if investment_dir.exists():
        for conn_dir in sorted(investment_dir.iterdir()):
            if not conn_dir.is_dir():
                continue
            for acct_file in sorted(conn_dir.glob("*.json")):
                if acct_file.name.endswith(".positions.json"):
                    acctid = acct_file.name[: -len(".positions.json")]
                    all_violations.extend(
                        validate_positions_file(acct_file, conn_dir.name, acctid)
                    )
                else:
                    all_violations.extend(
                        validate_investment_account_file(acct_file, conn_dir.name)
                    )

    # Shared securities registry
    securities_path = cache_dir / "securities.json"
    if securities_path.exists():
        all_violations.extend(validate_securities_file(securities_path))
